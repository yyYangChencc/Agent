...
class AgentConfig:
    ...
    relax_increase_rate: float = 0.1   # 任务为 "none"时每 tick relax 增加的量（满足 relax 需求的速率）
    relax_decay_rate: float = 0.05     # relax 需求的自然衰减速率（每 tick 减少的量）
    relax_moving_usage:float = 0.02       # agent 移动一步消耗的 relax 量
    satiety_decay_rate: float = 0.01        # 饱腹度的自然降低速率（每 tick 饱腹度降低的量）
