...
var textTokenization = require('../../text/canvas/utils/textTokenization.js');
...
function getBitmapTextLayout(chars, style, font, trimEnd) {
  ...
  if (shouldCollapseSpaces || shouldCollapseNewlines) {
    ...
    for (let c = 0; c < chars.length; c++) {
      ...
      if (textTokenization.isBreakingSpace(char)) {
        if (shouldCollapseSpaces && textTokenization.isCollapsibleSpace(char)) {
  ...
  const nextLine = () => {
    ...
    if (trimEnd) {
      ...
      while (textTokenization.isCollapsibleSpace(lastChar)) {
  ...
  for (let i = 0; i < chars.length + 1; i++) {
    ...
    if (isWordBreak) {
      ...
      if (textTokenization.isBreakAfterChar(char)) {
        const addWordToNextLine = !firstWord && style.wordWrap && checkIsOverflow(currentLine.width + currentWord.width);
        if (addWordToNextLine) {
          nextLine();
        }
