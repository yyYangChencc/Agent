...
- **网格世界**：固定使用 25×25 地图，支持移动、进食、睡眠、购买、进入建筑、离开建筑和近距离对话。
...
`backend/test_spatial.py` 是不调用 LLM 的空间交互演示服务，用于按固定步骤展示睡眠、进入建筑、离开建筑和建筑内移动。
...
| `eat_distance_sq` | `2.0` | 进食、睡眠、购买和进入建筑的平方距离限制 |
