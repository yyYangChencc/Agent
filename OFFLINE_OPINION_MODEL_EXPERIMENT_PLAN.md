...
1. 新模型是否比公式化线上/线下加权平均更新更能解释观念表征变化。
...
```text
M0：无心理中介基线
    只记录行为、发帖和观念评测，不启用需求压力到角色卡的调制

M1：加入 gap
    satisfaction -> gap -> behavior / assessment context

M2：加入 pressure_memory
    satisfaction -> gap -> pressure_memory -> effective_pressure -> behavior / assessment context

M3：加入 psychological mediators
    satisfaction -> gap -> pressure_memory -> effective_pressure -> mediators -> behavior / assessment context
...
```text
M4 应该表现出最强的行为可解释性。
M3 应该比 M0 更能解释观念评测分数和表达内容的变化来源。
...
观念评测分数变化必须能从需求压力、心理中介、角色卡、近期经历和社交文本中找到证据。
```
...
```text
如果社交网络完全相同，但 A组的观念评测分数、表达内容或互动方式不同，
说明内部心理状态进入了观念表征和表达过程。
```

该实验验证：

```text
internal state -> psychological mediators -> role card -> behavior / opinion assessment
```

## 5. 实验 4：同一内部状态，不同社交网络

目的：验证线下社交网络仍会通过经历、对话和社交文本进入观念评测上下文。
...
```text
A组和 B组应在互动证据、表达内容或观念评测理由上出现可解释差异。
```
...
```text
opinion_assessment score
opinion_assessment reason
转发/评论/点赞/点踩倾向
对新闻内容的表达方式
不同群体之间观念表征分化或收敛
```
...
```text
高 affiliation 组更容易在表达、点赞和评测理由中贴近高信任群体。
...
opinion_assessment_before
opinion_assessment_after
opinion_assessment_reason
opinion_assessment_evidence
psychological_context
recent_experiences
